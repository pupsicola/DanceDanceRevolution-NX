return Def.ActorFrame{
	Def.ActorFrame{
		InitCommand=cmd(player,PLAYER_1;x,-360;y,123);
		LoadActor("1P_OK")..{
		...
		--	ExitUnselectedP1Command=cmd(stoptweening;diffusealpha,0;zoomy,0);
			ExitSelectedP1Command=function(self)
				...
				(cmd(stoptweening;linear,0.1;diffusealpha,1;zoom,1.2;))(self)
			end;
		...
		LoadActor("1P_WAIT")..{
			...
			OffCommand=cmd(zoom,1.2;linear,0.1;zoomy,0);
		};
	};
	Def.ActorFrame{
		InitCommand=cmd(player,PLAYER_2;x,360;y,97.5);
		LoadActor("2P_OK")..{
			InitCommand=cmd(diffusealpha,0;zoomy,0);
